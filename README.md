...
세부 개념적인 내용은 아래와 같이 크게 4개로 나누어 설명합니다.
- ID(Identity) 스케폴더
- 데이터베이스 마이그레이션
- 엔터티프레임워크
- 주요 공급자(Provider) 인증

소스코드는 다음과 같이 2개의 버전으로 제공됩니다.
- ID(Identity) 스케폴더를 제공받아 구현하는 방식
- 빈 프로젝트에서 새로 구현하는 방식

#### 학습 순서에 대한 안내
> ❗ 빈 프로젝트를 통해 처음부터 구현하기 위해서는 ID(Identity) 스케폴더 구조가 충분히 숙련된 상태에서 시작하는 것이 좋습니다.
> 
> 그 이유는 스케폴더 구조가 생각보다 양이 많고 생소한 개념이 많기 때문에 진입장벽에 대한 어려움이 있을 수 있기 때문에 더더욱 스케폴더 구조를 자세하게 들여다보는 것을 권장합니다.
<br>
...
#### ✔️ 이 ID 모듈을 선택하는 이유는...
- 계정/인증 관련해서 어렵고 복잡한 구현 부분들을 도움 받을 수 있다
- 스케폴더 기법을 통해 원하는 부분만 편집이 가능
- DB 구조를 받을 수 있다 (마이그레이션)

#### ✔️ 하지만 ID(Identity) 구조를 반드시 사용해야 하는 것은 아닙니다.  
