...
Blazor는 닷넷프레임워크에서 벗어난 **닷넷코어** 기반으로 구성된 웹앱 환경입니다. 더이상 닷넷프레임워크에 의존하지 않는 닷넷코어는 동일한 소스코드를 사용하기 때문에 서로 다른 운영체제간 가능하며 기존 ASP.NET처럼 IIS에 의존하지 않고도 독립적인 호스팅이 가능합니다.
...
Blazor를 서비스하기 위한 가장 쉬운 방법은 클라우드 서비스를 이용하는 것입니다. 대표적인 클라우드 서비스로는 Azure와 AWS가 있습니다. Azure의 경우 무료로 웹앱을 서비스할 수 있기 때문에 개발단계에서 아주 유용하게 이용할 수 있습니다.
...
Authentication 인증을 최초에 성공하면 데이터베이스 연결 유무 및 마이그레이션 버전 정보 확인을 통해 마이그레이션이 시작됩니다. 그러므로 따로 직접 마이그레이션을 할 필요가 없습니다.
