...
**그리고** 이 모듈은 **Microsoft AspNetCore**에서 제공하는 신뢰있는 구조입니다.

### ID 모듈을 선택하는 이유는...
1. 계정/인증 관련 로직을 **AspNetCore** 팀으로 부터 가이드 받을 수 있다.
2. 스케폴더 기법을 통해 원하는 화면과 기능을 부분적으로 수정할 수 있다.
3. 데이터베이스 마이그레이션을 통해 DB 구조를 제공받을 수 있다.

하지만 ID(Identity) 구조를 반드시 사용해야 하는 것은 아닙니다.  
**예를 들어** 이미 사용중인 DB를 사용하는 경우 또는 독창적인 새 ID 인증 구조를 직접 설계하는 경우입니다.

예를 들어 필요 없는 구조가 포함된다거나 
특히 이 구조는 스케폴더 형태로 제공되기 때문에 필요한 부분만 손쉽게 구현할 수도 있습니다.

하지만 스케폴더 구조를 사용하는 것이 좋은 것 만은 아닙니다. 예를 들어 필요하지 않은 구조가 포함되거나 
